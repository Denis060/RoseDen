        ...
        <div className="mx-auto grid min-h-[420px] max-w-7xl grid-cols-[48%_52%] sm:min-h-[560px] lg:min-h-[680px] lg:grid-cols-[0.9fr_1.1fr]">
          <div className="z-10 flex flex-col justify-center px-4 py-8 sm:px-8 sm:py-12 lg:px-16">
            ...
            <div className="mt-5 flex max-w-[250px] flex-col gap-2 sm:mt-8 sm:max-w-none sm:flex-row sm:flex-wrap sm:gap-3">
              ...
              <a href={websiteWhatsappLink(content.whatsappNumber, "Hello RoseDen Atelier, I would like to see your latest arrivals.")} target="_blank" rel="noreferrer" className="flex h-10 items-center justify-center gap-1.5 rounded-md bg-burgundy px-3 text-[11px] font-bold text-white sm:h-14 sm:rounded-full sm:px-7 sm:text-base">
          ...
          <div className="relative min-h-full overflow-hidden">
            <Image src={content.heroImageUrl} alt="RoseDen Atelier" fill priority className="object-cover object-[67%_center]" />
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-5 grid grid-cols-3 gap-1 sm:mt-8 sm:gap-3">
            {atelierImages.slice(0, 3).map((image, index) => (
              <div key={`${image}-${index}`} className="relative aspect-[4/3] overflow-hidden rounded-lg border-2 border-white shadow-soft sm:rounded-3xl sm:border-4">
                <Image src={image} alt="Inside the RoseDen Atelier boutique" fill className="object-cover" />
              </div>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-6 grid grid-cols-5 gap-2 sm:mt-9 sm:gap-3">
            {channels.map(([Icon, title, text], index) => (
              <a key={title} href={index === 0 ? websiteWhatsappLink(content.whatsappNumber, "Hello RoseDen Atelier, I would like to place an order.") : index === 4 ? "/shop" : "/contact"} target={index === 0 ? "_blank" : undefined} rel={index === 0 ? "noreferrer" : undefined} className="rounded-xl border border-gold/20 bg-cream px-1 py-3 text-center sm:rounded-2xl sm:p-5">
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-7 grid grid-cols-5 gap-1 sm:mt-9 sm:gap-4">
            {services.map(([Icon, fallbackTitle, fallbackText], index) => {
              ...
              return <div key={service.title} className="border-r border-gold/25 px-1 text-center last:border-r-0 sm:px-4">

...
export default function AdminReportsPage() {
  ...
  return (
    <div>
      ...
      <section className="mt-7"><h2 className="mb-1 font-display text-xl font-semibold text-wine">Best-selling products</h2><p className="mb-3 text-xs text-black/45">The stock Rosannah should consider buying again.</p><div className="space-y-3 rounded-2xl bg-white p-4 shadow-soft">{productSales.length === 0 && <EmptyReport>No product-linked sales for this month.</EmptyReport>}{productSales.slice(0, 6).map((item, index) => <div key={item.id}><Link href={`/admin/inventory/${item.id}`} className="flex items-center justify-between gap-3 text-sm"><span className="flex items-center gap-2"><Star size={15} className="text-gold" />{index + 1}. {item.name}<small className="text-black/40">{item.count} sold</small></span><span className="text-right"><strong className="block text-burgundy">{money(item.revenue)}</strong><small className="text-emerald-700">{money(item.profit)} gross profit</small></span></Link><ProgressBar value={item.count} max={maxProductCount} tone="gold" /></div>)}</div></section>
    </div>
  );
}

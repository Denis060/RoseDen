...
export function AdminGate({ children }: { children: React.ReactNode }) {
  const { loading, mode, roleLoading, userEmail, userRole } = useData();
  const router = useRouter();
  const pathname = usePathname();

  useEffect(() => {
    if (mode === "supabase" && !loading && !userEmail) router.replace(`/login?next=${encodeURIComponent(pathname)}`);
  }, [loading, mode, pathname, router, userEmail]);

  if (loading || roleLoading || (mode === "supabase" && !userEmail)) {
